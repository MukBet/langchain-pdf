from app.chat.redis import redis_client as client
import random

def random_component_by_score(
    component_type, component_map
):
    if(component_type not in ["llm", "retriever", "memory"]):
        raise ValueError("Invalid component type")
    
    value = client.hgetall(f"{component_type}_score_values")
    count = client.hgetall(f"{component_type}_score_counts")

    names = component_map.keys()
    scores = {}
    for name in names:
        core = int(value.get(name, 1))
        cnt = int(count.get(name, 1))
        avg = core / cnt
        scores[name] = max(avg, 0.1)  # Ensure a minimum score of 0.1
